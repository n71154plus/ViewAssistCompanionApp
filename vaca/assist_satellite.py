...
class ViewAssistSatelliteEntity(WyomingAssistSatellite, VASatelliteEntity):
    ...
    def on_receive_event_callback(self, event: Event) -> tuple[bool, Event | None]:
        ...
        if event and CustomEvent.is_type(event.type):
            ...
            if evt.event_type == CAPABILITIES_EVENT_TYPE and evt.event_data:
            ...
            elif evt.event_type == BLE_ADVERTISEMENT_EVENT_TYPE and evt.event_data:
                # Forward BLE advertisement to HA bluetooth integration
                ble_scanner = self.hass.data.get(f"{DOMAIN}_ble", {}).get(self.config_entry.entry_id)
                if ble_scanner is not None:
                    ble_scanner.inject_advertisement(evt.event_data)
                ...
                # Forward BLE GATT result to the proxy
                gatt_proxy = self.hass.data.get(f"{DOMAIN}_gatt", {}).get(
                    self.config_entry.entry_id
                )
                if gatt_proxy is not None and evt.event_data is not None:
                    gatt_proxy.handle_result(evt.event_type, evt.event_data)
    ...
    async def _connect(self) -> None:
        ...
        # Wire the GATT proxy's outgoing path to this client connection
        gatt_proxy = self.hass.data.get(f"{DOMAIN}_gatt", {}).get(
            self.config_entry.entry_id
        )
        if gatt_proxy is not None:
            def _send_ble_event(event_type: str, data: dict) -> None:
                if self._client and self._client.can_write_event():
                    self.config_entry.async_create_background_task(
                        self.hass,
                        self._client.write_event(CustomEvent(event_type, data).event()),
