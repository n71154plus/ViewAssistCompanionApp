...
class VacaHttpServer(
    ...
    private fun handleBlePage(socket: Socket) {
        ...
        sb.append("if(!d.devices||d.devices.length===0){tb.innerHTML='<tr><td colspan=\"4\" class=\"empty\">'+t('ble_no_devices')+'</td></tr>';return;}")
    ...
    private fun handleBleDevices(socket: Socket) {
        ...
        devices.forEachIndexed { i, d ->
            if (i > 0) sb.append(",")
            val name = d.name.replace("\\", "\\\\").replace("\"", "\\\"")
            sb.append("{\"address\":\"${d.address}\",\"name\":\"$name\",\"rssi\":${d.rssi}")
            sb.append(",\"last_seen\":${d.lastSeen}")
            sb.append(",\"service_uuids\":[${d.serviceUuids.joinToString(",") { "\"$it\"" }}]")
