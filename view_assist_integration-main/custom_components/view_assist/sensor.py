...
class ViewAssistSensor(RestoreSensor):
        ...
        if last_sensor_data:
            ...
            if last_state and last_state.attributes:
                ...
                system_managed_attrs = {
                    ...
                    "use_announce", "app_installed_list", "app_installed_sensor",
                    "app_installed_entry_id",
    ...
    def _get_display_device_status_attributes(self) -> dict[str, Any]:
        ...
        return {
            ...
            "app_installed_sensor": d.core.app_installed_sensor,
            "app_installed_entry_id": self._get_vaca_entry_id(d.core.app_installed_sensor),
            "app_installed_list": self._get_app_installed_list(d.core.app_installed_sensor),
        }

    def _get_vaca_entry_id(self, sensor_entity_id: str | None) -> str | None:
        """Get the VACA config entry ID from the app_installed_sensor entity."""
        if not sensor_entity_id:
            return None
        entity_reg = er.async_get(self.hass)
        if entity := entity_reg.async_get(sensor_entity_id):
            return entity.config_entry_id
        return None

    def _get_app_installed_list(self, sensor_entity_id: str | None) -> list:
        """Read installed apps list from the linked VACA sensor entity."""
        if not sensor_entity_id:
            return []
        if state := self.hass.states.get(sensor_entity_id):
            return state.attributes.get("installed_apps", [])
        return []
