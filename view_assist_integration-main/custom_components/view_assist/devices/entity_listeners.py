...
class EntityStateChangedHandler:
    ...
    def register_listeners(self) -> None:
        ...
        # Add listener for VACA installed apps sensor changes
        if app_installed_sensor := self.config.runtime_data.core.app_installed_sensor:
            self._add_entity_state_listener(
                app_installed_sensor, self._async_on_app_installed_sensor_change
            )
    ...
    @callback
    def _async_on_app_installed_sensor_change(
        self, event: Event[EventStateChangedData]
    ) -> None:
        """Handle VACA installed apps sensor state changes."""
        ...
        # Only dispatch update if the installed_apps attribute actually changed
        new_apps = new_state.attributes.get("installed_apps")
        old_apps = old_state.attributes.get("installed_apps") if old_state else None

        if new_apps != old_apps:
            _LOGGER.debug(
                "Installed apps changed on %s, refreshing VA sensor",
                new_state.entity_id,
            )
            async_dispatcher_send(
                self.hass,
